...
static inline void loadBar(size_t x, size_t n, int r, int w)
{
    ...
    // Rprintf("]\n33[F33[J");
    Rprintf("]\r"); // Move to the first column
...
size_t analysis(short **incidence,size_t ncol,size_t nrow,double **scores,size_t step,size_t max_iter,size_t verbose)
{
  ...
  do matrix=(short **)R_alloc(nrow,sizeof(short*)); while(matrix==NULL);
	for(i=0;i<nrow;++i)
	{ 	
		do	matrix[i]= (short*)R_alloc(ncol,sizeof(short)); while(matrix[i]==NULL);
    for(j=0;j<ncol;++j)
			{
	...
	do	*scores=(double*)R_alloc(dim,sizeof(double));	while(scores==NULL);
	for(i=0;i<dim;(*scores)[i++]=0.0);
	(*scores)[0]=1.0;
 	do	from=(size_t*)R_alloc(e,sizeof(size_t));	while(from==NULL);
	do	to=(size_t*)R_alloc(e,sizeof(size_t));	  while(to==NULL);
 	kk=0;
	for(i=0;i<nrow;++i)
		for(j=0;j<ncol;++j)
			if(matrix[i][j]==1)
...
size_t inline check(size_t *pos,size_t *to,size_t *index,size_t a,size_t  b,size_t  c,size_t d,size_t e,size_t nr)
{
    size_t i;
...
size_t rewire_sparse_bipartite(size_t *from,size_t *to,size_t nc,size_t nr,size_t max_iter,size_t e,size_t verbose)
{
	...
	size_t *index;
	size_t *pos;
	do	index = (size_t*)R_alloc( (nr+1),sizeof(size_t ));	while(index==NULL);
	do	pos = (size_t*)R_alloc(e,sizeof(size_t ));	 while(pos==NULL);
	index[0]=0;
  pos[0]=0;
	kk=1;
  j=0;
...
void inline sub(size_t a,size_t b,size_t c,size_t d,size_t *degree,short **adj)
{
    size_t i;
...
void inline sub2(size_t a,size_t b,size_t c,size_t d,size_t *degree,short **adj)
{
    size_t i;
...
size_t rewire_sparse(size_t *from, size_t *to,size_t *degree,size_t ncol, size_t nrow,size_t max_iter, size_t e,size_t verbose)
{
	...
	size_t ad,cb,ac,bd;
	short **adj;
	do	adj=(short **)R_alloc(nrow,sizeof(short*)); while(adj==NULL);
	for(i=0;i<nrow;i++)
    {
       do	adj[i]= (short*)R_alloc(degree[i]+1,sizeof(short)); while(adj[i]==NULL);
