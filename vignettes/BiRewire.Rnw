...
\end{verbatim}
We suggest to use the biocLite function from R in order to have the last working relase package (build and check procedure).
...
BiRewire requires the R packages {\bf Matrix} {\bf igraph}\cite{igraph}, {\bf{slam}}\cite{slam} and {\bf tsne}\cite{tsne} available at the CRAN repository.
...
#res=birewire.analysis.bipartite(get.incidence(ggg,sparse=F),
#				max.iter=10*N,n.networks=10)
#and monitoring the markov chain
#birewire.visual.monitoring.bipartite(ggg,display=T,n.networks=75,
#				sequence=c(1,10,200,500,"n",10000),ncol=3)
...
\bibitem{Iorio} Iorio, F. and Gobbi, A. (2016) \emph{In preparation} In preparation.
