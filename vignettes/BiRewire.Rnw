...
Moreover, with se same routines, it is possible to generate a rewired version of a given directed signed network (DSG), encoding for example a patway or a signaling network (for details see section \ref{sec:DSG} and \cite{Iorio}). 
...
A directed signed network (DSG) $G$ is a directed network in which the edges are encoded with a triplet $(a,b,*)$ where $a$ denotes the source node, $b$ the target node and $*$ the sign of the relation (the weight of the edge). In our case (pathways and signaling) $*$ can be positive $+$ and negative $-$.
...
\subsection{{\bf birewire.analysis.bipartite} and{\bf birewire.analysis.undirected} }
...
scores<-birewire.analysis.bipartite(BRCA_binary_matrix,step,verbose=FALSE,max.iter=max,n.networks=5,display=)

@

The function \textbf{birewire.analysis.bipartite} returns the Jaccard similarity sampled every \textit{step} SSs (in the example above step is equal to 5000). In the resulting plots the value of the analyitically derived lower bound to the number of switching steps is also visualised {\bf \$N }. For more details see the the documentation.\\ 
...
scores.und<-birewire.analysis.undirected(m.und,step=step,verbose=FALSE,max.iter=max,n.networks=5)
...
\subsection{{\bf birewire.visual.monitoring.bipartite  } and {\bf birewire.visual.monitoring.undirected}}
...
This function uses the SA to generate a set of $K$ DSG in SIF format drawn from the null model given by an initial DSG. The function creates a main folder (\textit{path} input parameter) and a set of subfolders in order to have maximum 1000 files per folder.

<<Sampler DSG>>=
##NOT RUN
##birewire.sampler.dsg(dsg,K=10,path='TESTBIREWIREDSG',verbose=F,delimitators=list(negative='-',positive='+'))
